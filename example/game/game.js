...
const CONFIG = {
  ...
  // --- Бабл ---
  // пути относительны HTML-странице (game/index.html). Картинки — в img/ этого же проекта.
  spriteFile: "img/bubble.png",
  ...
  // mechanic — имя механики на кадре (см. mechanics/). Без mechanic —
  // кадр без «трения», переход открыт сразу (старт / финал).
  // magnet — спрайт магнитика-награды кадра.
  SCENES: [
    { image: "img/scene_1.png" },                          // квартира — старт
    {
      // Италия — мини-башня стоит на столе перед пастой
      image: "img/scene_2.png", mechanic: "seek",
      magnet: "img/magnet_italy.png",
      seek: { x: 0.26, y: 0.74 },
    },
    {
      // Америка — Статуя стоит рядом с тележкой хот-догов
      image: "img/scene_3.png", mechanic: "fog",
      magnet: "img/magnet_usa.png",
      fog: { x: 0.54, y: 0.78 },
    },
    {
      // Япония — драккар бегает (старт-позиция любая)
      image: "img/scene_4.png", mechanic: "chase",
      magnet: "img/magnet_japan.png",
      chase: { x: 0.55, y: 0.40 },
    },
    {
      // Норвегия — кусочки разбросаны по воде слева от существующего кораблика;
      // собранный драккар появляется чуть выше, тоже на воде.
      image: "img/scene_5.png", mechanic: "collect",
      magnet: "img/magnet_norway.png",
      collect: [
    ...
    },
    {
      ...
      image: "img/scene_6.png", mechanic: "fridge",
...
window.addEventListener("keydown", (e) => {
  ...
  // E (по e.code — работает в любой раскладке) — общее действие, см. handleAction()
  if (e.code === CONFIG.transitionCode) handleAction();
});

// действие (клавиша E / тач-кнопка): сначала пробуем подобрать магнитик
// в механике; если подбирать нечего — это команда перехода.
...
// press/release/action — для экранных тач-кнопок (index.html):
// эмулируют зажатые клавиши через тот же объект keys, что читает update()
window.GameAPI = {
  restart,
