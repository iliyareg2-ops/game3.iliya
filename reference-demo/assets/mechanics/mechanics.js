/*
  ...
  Механики регистрируются по имени. Движок (game.js) создаёт нужную через
...
/*
  createMagnet — общий объект магнитика для механик с одной целью
  (seek / fog / chase). Держит спрайт, рисует с мерцанием, проверяет
  дистанцию для подбора на E, проигрывает анимацию подбора (вспышка).

  cfg — CONFIG, sprite — путь к картинке магнитика (scene.magnet).
*/
function createMagnet(cfg, sprite) {
  const img = new Image();
  let loaded = false;
  ...
  // размеры на экране с сохранением пропорций: высота = cfg.magnetSize,
  // ширина — пропорционально (узкая башня узкая, широкий драккар широкий).
  function size() {
    const h = cfg.magnetSize;
    return { w: h * aspect, h };
  }

  return {
    x: 0, y: 0,
    visible: false,      // показывать ли магнитик (механика включает)
    picked: false,       // подобран
    _pulse: 0,
    _anim: 0,            // прогресс анимации подбора 0..1

    // Бабл достаточно близко для подбора на E?
    inReach(bubble) {
    ...
    draw(ctx) {
      ...
      if (this.picked) {
        // анимация подбора: магнитик всплывает, растёт, тает + вспышка
        const t = this._anim;
        const scale = 1 + t * 0.6;
        const lift = t * h * 0.7;
        ctx.save();
